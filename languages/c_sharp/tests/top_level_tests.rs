...
use uast::{TopLevel, FunctionDef, ClassDef, FunctionBody, Statement, ReturnStatement, Expression, Literal};
...
fn test_lower_class_with_method() {
    ...
    if let TopLevel::Class(class_def) = result {
        ...
        if let TopLevel::Function(func_def) = &body[0] {
            ...
            // Check parameters
            let params = func_def.parameters.as_ref().expect("Parameters should be present");
            assert_eq!(params.len(), 1);
            assert_eq!(params[0].name, "a");
            assert_eq!(params[0].var_type, Some("int".to_string()));

            // Check body
            let func_body = func_def.body.as_ref().expect("Function body should be present");
